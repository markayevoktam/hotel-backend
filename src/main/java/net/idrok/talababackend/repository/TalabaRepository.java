...
TalabaRepository extends JpaRepository<Talaba, Long> {
    ...
    @Query("FROM Talaba t WHERE upper(t.ism) like concat('%',:key,'%') OR upper(t.familya) like concat('%',:key,'%') OR upper(t.sharif) like concat('%',:key,'%') OR  upper(t.hudud) like concat('%',:key,'%')   ")
